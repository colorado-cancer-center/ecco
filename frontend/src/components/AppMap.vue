...
function bindPopup(layer: L.Layer) {
  ...
  layer.on("popupopen", async (event) => {
     ...
     * effects.
     */
    let info = cloneDeep(event.sourceTarget.feature.properties);
...
/** update data layers */
function updateData() {
  /** type-safe get properties */
  const getProp = (feature: Feature, prop: string, type: string) =>
    feature.properties &&
    prop in feature.properties &&
    typeof feature.properties[prop] === type;

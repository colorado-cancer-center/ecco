    ...
    for model in family:
        ...
        def generate_routes(type=type, model=model, simple_model_name=simple_model_name):
            ...
            @router.get(
                ...
                For cancer models, the 're' and 'sex' arguments filter race/ethnicity
                and sex by the given arguments, respectively; by default, they're
                both "All".
                """
            )
            async def get_dataset_fips(
                measure: str, re : str = "All", sex : str = "All",
                ...
                if model not in CANCER_MODELS:
                    ...
                    query = select((model.FIPS, model.AAR.label("value"), model.AAC.label("aac"))).where(
                        model.Site == measure,
                        model.RE == re,
                        model.Sex == sex
                    )

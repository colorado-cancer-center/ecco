...
@router.get(f"/measures", response_model=None) # dict[str,dict[str,MeasuresMetaResponse]])
async def get_dataset(session: AsyncSession = Depends(get_session)):
    f"""
    ...
    all_measures = defaultdict(lambda: defaultdict(str))
    ...
    for type, models in STATS_MODELS.items():
        for model in models:
            ...
            print(f"Fetching measures for {simple_model_name}...")

            result = await session.execute(query)
            all_measures[type][simple_model_name] = {
    ...
    for model in family:
        ...
        def generate_routes(model=model):
            ...
            async def get_dataset_measures(session: AsyncSession = Depends(get_session)):
                ...
                # check if model is an instance of CancerStatsByCounty
            ...
            async def get_dataset(
                ...
                """
                Autogenerated method; gets all rows from the specified {model.__name__} table.
                """
